...
func (cg *CodeGen) emitHeader() {
	...
	cg.emit("    cmp $10, %%al")
	cg.emit("    jne concat_int_suffix_copy")
	cg.emit("    cmpb $0, 1(%%rsi)")
	cg.emit("    je concat_int_suffix_done")
	...
	cg.emit("    cmp $10, %%al")
	cg.emit("    jne concat_cstr_prefix_copy")
	cg.emit("    cmpb $0, 1(%%r10)")
	cg.emit("    je concat_cstr_prefix_done")
	...
	cg.emit("    cmp $10, %%al")
	cg.emit("    jne concat_cstr_cstr_copy_left_char")
	cg.emit("    cmpb $0, 1(%%r10)")
	cg.emit("    je concat_cstr_cstr_left_done")
	...
	cg.emit("    cmp $10, %%al")
	cg.emit("    jne concat_cstr_cstr_copy_right_char")
	cg.emit("    cmpb $0, 1(%%r11)")
	cg.emit("    je concat_cstr_cstr_right_done")
